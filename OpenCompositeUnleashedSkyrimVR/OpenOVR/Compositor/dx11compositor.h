...
protected:
	...
	// FSR upscaling (two-pass: EASU edge-adaptive upscale + RCAS contrast-adaptive sharpening)
	ID3D11VertexShader* fsr_vshader = nullptr;
	ID3D11PixelShader* fsr_pshader = nullptr;   // EASU (upscale)
	...
	// Intermediate texture: EASU writes here, RCAS reads from here
	ID3D11Texture2D* fsrIntermediate = nullptr;
	ID3D11ShaderResourceView* fsrIntermediateSRV = nullptr;
	ID3D11RenderTargetView* fsrIntermediateRTV = nullptr;
