...
private:
	struct event_info_t {
		...
		event_info_t(vr::VREvent_t ev, vr::TrackedDevicePose_t pose)
		    : ev(ev), pose(pose) {}
	};
...
public:
	...
	void _OnPostFrame();
	void _EnqueueEvent(const vr::VREvent_t& e);
...
public:
	...
	* This pose will always be older than the call to this function and should not be used to render the device.
	 ...
	 * is invalid. When the VR keyboard laser is on the keyboard, the trigger is masked from this call. */
	...
	/** Same as GetControllerState but never masks any buttons. Used by the VR keyboard
	 * so it can read its own trigger presses without being affected by the masking. */
	bool GetUnmaskedControllerState(vr::TrackedDeviceIndex_t unControllerDeviceIndex, vr::VRControllerState_t* pControllerState, uint32_t unControllerStateSize);
