...
// doesn't double-process menu clicks (prevents menu stacking).
extern bool g_menuLaserActive;
...
bool BaseSystem::GetUnmaskedControllerState(vr::TrackedDeviceIndex_t controllerDeviceIndex,
    vr::VRControllerState_t* controllerState, uint32_t controllerStateSize)
{
	if (sizeof(VRControllerState_t) != controllerStateSize)
		OOVR_ABORT("Bad controller state size");
