...
extern float g_fsr3JitterY;
extern bool g_fsr3JitterEnabled;
extern int g_fsr3JitterPhaseCount;
// Camera near/far — defined in dx11compositor.cpp, written here
extern float g_fsr3CameraNear;
extern float g_fsr3CameraFar;
#endif
...
vr::HmdMatrix44_t XrHMD::GetProjectionMatrix(vr::EVREye eEye, float fNearZ, float fFarZ, EGraphicsAPIConvention convention)
{
	...
	if (eEye == vr::Eye_Left) {
		...
		g_fsr3CameraFar = fFarZ;
		{ static bool s = false; if (!s) { s = true;
			OOVR_LOGF("FSR3: Game near=%.2f far=%.1f (captured from GetProjectionMatrix)", fNearZ, fFarZ); } }
	}
#endif
...
#ifdef OC_HAS_FSR3
		// FSR 3 temporal upscaling: apply sub-pixel jitter to the projection matrix.
		// This dithers the sample pattern across frames so FSR can reconstruct detail.
